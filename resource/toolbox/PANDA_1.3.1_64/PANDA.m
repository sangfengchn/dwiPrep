...
if ismac
    try
        system(['sysctl -n machdep.cpu.core_count > ' pwd filesep 'PANDA_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_System.txt']);
        delete([pwd filesep 'PANDA_System.txt']);
    catch
        QuantityOfCpu = '';
    end
elseif isunix
    try
        system(['cat /proc/cpuinfo | grep processor | wc -l > ' pwd filesep 'PANDA_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_System.txt']);
        delete([pwd filesep 'PANDA_System.txt']);
    catch
        QuantityOfCpu = '';
...
if ~isempty(QuantityOfCpu)
    pipeline_opt.max_queued = QuantityOfCpu;
else
        ...
        if ~LogPathPermissionDenied
            ...
            switch button
                case 'Yes'
                    ...
                    fclose(fid);
                    system(['which sh > ' pwd filesep 'PANDA_System.txt']);
                    fid = fopen([pwd filesep 'PANDA_System.txt']);
                    ShPath = fgets(fid);
                    fclose(fid);
                    delete([pwd filesep 'PANDA_System.txt']);
...
%                         system(instr_batch);
                        system(['which sh > ' pwd filesep 'PANDA_System.txt']);
                        fid = fopen([pwd filesep 'PANDA_System.txt'], 'r');
                        ShPath = fgets(fid);
                        fclose(fid);
                        delete([pwd filesep 'PANDA_System.txt']);
                        system([ShPath(1:end-1) ' ' CalculateJobStatusMainShLocation ' &']);
