...
if ismac
    try
        system(['sysctl -n machdep.cpu.core_count > ' pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
        delete([pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
    catch
        QuantityOfCpu = '';
    end
elseif isunix
    try
        system(['cat /proc/cpuinfo | grep processor | wc -l > ' pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
        delete([pwd filesep 'PANDA_DICOMTONIfTI_System.txt']);
    catch
        QuantityOfCpu = '';
...
if ~isempty(QuantityOfCpu)
    DICOMNIfTI_Pipeline_opt.max_queued = QuantityOfCpu;
else
