...
if ~isfield(pipeline_opt,'max_queued')
    if ismac
        try
            system(['sysctl -n machdep.cpu.core_count > ' pwd filesep 'Pipeline_Opt_System.txt']);
            QuantityOfCpu = load([pwd filesep 'Pipeline_Opt_System.txt']);
            delete([pwd filesep 'Pipeline_Opt_System.txt']);
        catch
            QuantityOfCpu = '';
        end
    elseif isunix
        try
            system(['cat /proc/cpuinfo | grep processor | wc -l > ' pwd filesep 'Pipeline_Opt_System.txt']);
            QuantityOfCpu = load([pwd filesep 'Pipeline_Opt_System.txt']);
            delete([pwd filesep 'Pipeline_Opt_System.txt']);
        catch
            QuantityOfCpu = '';
