...
if ismac
    try
        system(['sysctl -n machdep.cpu.core_count > ' pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
        delete([pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
    catch
        QuantityOfCpu = '';
    end
elseif isunix
    try
        system(['cat /proc/cpuinfo | grep processor | wc -l > ' pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
        QuantityOfCpu = load([pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
        delete([pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
    catch
        QuantityOfCpu = '';
...
if ~isempty(QuantityOfCpu)
    BetT1_Pipeline_opt.max_queued = QuantityOfCpu;
else
...
%                     system(instr_batch);
                    system(['which sh > ' pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
                    fid = fopen([pwd filesep 'PANDA_T1BrainExtraction_System.txt'], 'r');
                    ShPath = fgets(fid);
                    fclose(fid);
                    delete([pwd filesep 'PANDA_T1BrainExtraction_System.txt']);
