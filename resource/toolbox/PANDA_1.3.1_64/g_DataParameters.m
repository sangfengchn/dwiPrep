...
try 
    ...
    if length(DataRaw_cell) == 3
        ...
        system(['fslinfo ' NII_Path{1} ' > ' ScanningParameterFolder filesep  'ScanningParameters_System.txt']);
        fid = fopen([ScanningParameterFolder filesep  'ScanningParameters_System.txt'], 'r');
        delete([ScanningParameterFolder filesep  'ScanningParameters_System.txt']);
        num = 0;
        while ~feof(fid)
            Info_Str = fgets(fid);
